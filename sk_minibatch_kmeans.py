...
parser.add_argument('--batch_size',type=int,default=100,help='Size of the mini batches.')

parser.add_argument('--compute_labels',type=bool,default=True,help='Compute label assignment and inertia for the complete dataset once the minibatch optimization has converged in fit.')

parser.add_argument('--reassignment_ratio',type=float,default=0.01,help='Control the fraction of the maximum number of counts for a center to be reassigned. A higher value means that low count centers are more easily reassigned, which means that the model will take longer to converge, but should converge in a better clustering.')

parser.add_argument('--max_no_improvement',type=int,default=10,help='Control early stopping based on the consecutive number of mini batches that does not yield an improvement on the smoothed inertia. To disable convergence detection based on inertia, set max_no_improvement to None.')

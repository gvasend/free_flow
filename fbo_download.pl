...
dir_loc(fbo_files, '/home/gvasend/nfs/Simbolika/fexbiz/files').
...
solicitation_link(Sol, Link) :-
	once(fcypher_query('match (n:FBO_Announcement {SOLNBR: "~w"}) return n.LINK', [Sol], row([Link]))),
...
% got same bad data with \n at end of solnbr. delete this when it is cleaned up.
	once(fcypher_query('match (n:FBO_Announcement {SOLNBR: "~w\n"}) return n.LINK', [Sol], row([Link]))),
	...
	!.
solicitation_link(Sol, Link) :-
	once(fcypher_query('match (n:FBO_Announcement {SOLNBR: "~w"}) return n.URL', [Sol], row([Link]))),
...
% Analyze and catalog fbo files and compile results into gdb
	...
	append([], ['*.pdftext', '*image*.*','info.txt', '*annie*.*','*.ignore','*.pl'], Exclude),
	greplite:walk_files([fedbizopps:analyze_file], ['*.*'], Exclude, ['/home/gvasend/nfs/Simbolika/fedbiz/Files'], Hits, [output=escape]),
    format('before write\n',[]),
...
fcat_analyze_file(File) :-
	categorize_file(File, _, Type),
    fcypher_query('match (f:file) where f.iname="~w" set f.file_type="~w"',[File,Type],_).
...
file_type([instructions], instructions).
file_type([question,answer,'q&a'], q_and_a).
file_type(['*justification*and*approval*'],j_and_a).
...
file_type([pws,sow,'*statement*of*work','*performance*work*statement'], pws).
file_type([rfi,'*request*for*information*'], rfi).
file_type([rfp], rfp).
file_type([rfq],rfq).
file_type([attendee,attendance,roster], attendee_list).
file_type([cost, pricing], pricing).
file_type([soo,requirements,'*statement*of*objectives*'], requirements).
file_type([conference], conference).
file_type(['*past*perf*'], past_performance).
file_type(['*section?l*'], section_l).
file_type(['*section?m*'], section_m).
file_type(['*system*specification*'], system_spec).
file_type([conops,'*concept*of*operations*'], conops).
file_type(['*sole*source*'], sole_source).
file_type([plan],plan).
file_type([solicitation],solicitation).
...
download_window([window(1-5,0,12,20.0),window(1-5,12,20,300.0),window(6-7,0,20,10.0)]).
...
create_download_queue2(ProcessDate) :-
    working_directory(OldDir,'/home/gvasend/nfs/Simbolika/fedbiz'),
% Create a download queue that consists of each solicitation that has a document_package (should mean it has files)
% There are lots of directories with no files. Need to figure out why.
    findall(SOL, 
        fcypher_query('MATCH (n:DATE {iname:"~w"})-[]-(:FBO_Announcement)-[]-(s:SOLNBR)  RETURN s.iname', [ProcessDate], row([SOL])),
        SolList),
	list_to_set(SolList, SolSet),
    format('solset:~w\n',[SolSet]),
	...
	(
		member(S, SolSet),
		get_time(T0),
		new_node(_, check, [type=download,priority=Pri, date=ProcessDate,solnbr=S,time=T0,status=pending]),
	fail;true
	),
	true.
...
download_solicitation(SolNbr,Date) :-
	...
	(
		member(file(Name,File), Files),
		atomic_concat('https://www.fbo.gov',File,URL),
		atomic_list_concat([AbsDir, '/',Name], FullName),
        categorize_file(Name, _, DocType),
        format('~w -> ~w\n',[Name,DocType]),
        new_node(FullName, file, [filename=File, url=URL, solnbr=SolNbr, doc_type=DocType]),
        once(
         	fcypher_query('match (n:SOLNBR) where n.iname="~w" return id(n)',[SolNbr],row([SolId]))
        ;
            SolId=SolNbr
        ),
        new_edge(_, SolId, FullName, hasFile, []),
    fail;true),
	(
		member(file(Name,File), Files),
		atomic_concat('https://www.fbo.gov',File,URL),
		atomic_list_concat([AbsDir, '/',Name], FullName),
		once((\+ exists_file(FullName) ; exists_file(FullName), (log_message(info('File already exists\n',FullName)), increment(file_exists)), fail)),
		increment(download_file_attempt),
		download_file(URL,FullName),
		get_time(T0),
		new_node(_, check, [type=fcat,status=pending,date=Date,solnbr=SolNbr,url=URL,filename=FullName,created=T0]),
	fail;true).
...
get_next_solicitation(Id,Solicitation,Date) :-
	increment(get_next_solicitation),
	once(
			fcypher_query('match (ck:check{type: "download", status: "pending"}) return id(ck),ck.solnbr, ck.date order by ck.priority',[],row([Id,Solicitation,Date]))
			),
    log_message(info('Found SOL')),
...
download_manager :-
    ...
    log_message(info('download window open')),
    log_message(info('connection test')),
	check_connection,
    log_message(info('connection available')),
	get_next_solicitation(Id,Solicitation0,Date),
    ...
    format('check solicitation ~w\n',[Solicitation]),
	log_messageF(info(dl_check_sol,Id,Solicitation)),
	catch( ignore(download_solicitation(Solicitation,Date)), Error, 
	(
   	...
   	fcypher_query('match (c:check) where c.solnbr="~w" and c.type="download" and c.status="pending" detach delete c',[SolNbr],_),
...
check_connection1 :-
	...
	catch( http_client:http_get('http://whatismyipaddress.com/', _, []), _, lost_connection(Old)), 
	(
		Old=1,New=0,
...
download_window_open :-
	...
	flag(download_manager_running,_,0),                % turn dm off
    format('download window closed\n',[]),
	log_message(info('download window not open')),
	sleep(60.0), fail.
...
cat_files  :-
    greplite:walk_files([fedbizopps:catalog_file], ['*.*'], [], ['/home/gvasend/nfs/Simbolika/fedbiz/Files'], _, []),

catalog_file() :-

% CREATE CONSTRAINT ON (f:file) ASSERT f.iname IS UNIQUE

:- add_graph_dest(neo4j).			% always using neo4j at the moment.
	
:- create_log.

dm_on :- flag(download_manager, _, 1).
dm_off :- flag(download_manager, _, 0).

dm_is_on :- flag(download_manager, 1, 1).
dm_is_off :- flag(download_manager, 0, 0).



:- dm_on, download_manager, dm_is_off, halt.
